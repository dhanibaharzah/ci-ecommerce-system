 ...
 ?>
    ...
    <script>
        ...
        $("#btnForApplyCouponCheckoutPage").on("click", function(){
            ...
            $("#msgSuccessCouponCheckout").hide();
            const ongkir = $("#ongkirValueOrder").val();
            if(ongkir == "0"){
                $("#msgDangerCouponWrongCheckoutPage").text("Pilih metode pengiriman dulu");
            }else{     
                if(coupon != ""){
                    $.ajax({
                        url: "<?= base_url(); ?>payment/checkoutCoupon",
                        type: "post",
                        dataType: "json",
                        async: true,
                        data: {
                        ...
                        success: function(data){
                            ...
                            if(data.success == true){
                                ...
                                $("#idCouponCheckoutPage").val(data.id_coupon);
                                const courier = $("#ongkirValueOrder").val();
                                const price = $("#paymentPriceTotalAll").val();
                                const potongan = $("#potonganCouponCheckoutPageValue").val();
                                const total = parseInt(price) + parseInt(courier) - parseInt(potongan);
                                $("#paymentTotalAll").text("Rp"+number_format(total).split(",").join("."));
                                $("#msgSuccessCouponCheckout").show();
                            }else{
