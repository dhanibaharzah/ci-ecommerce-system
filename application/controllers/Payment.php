...
class Payment extends CI_Controller {
    ...
    public function checkoutCoupon(){
        ...
        $coupon_use = $this->db->get_where('coupon_use', ['coupon' => $check['id'], 'user' => $this->session->userdata('id')]);
        ...
        if($check){
            if($coupon_use->num_rows() < $check['limit_user']){
                if($check['dated'] >= date('Y-m-d')){
                    if($totalall >= $check['min']){
                        echo json_encode(["success" => true, "id_coupon" => $check['id'], "discount" => $check['discount']]);
                    }else{
                ...
                echo json_encode(["success" => false, "msg" => "Kupon tidak bisa digunakan lagi karena sudah melebihi batas pemakaian"]);
            }
        }else{
